...
from .utils import quantize_per_channel, quantize_per_tensor, ref_per_channel_quant_gemm
from ..utils import acc_check, bench_by_secs, get_device


@triton.jit
def per_channel_quant_gemm_kernel(
    x_ptr,
    w_ptr,
    o_ptr,
    x_scale_ptr,
...
def main():
    ...
    # Quantize activations per tensor and weights per output channel
    X_q, x_scale = quantize_per_tensor(X)
    W_q, w_scales = quantize_per_channel(W)

    # Compute ref output
    y_ref = ref_per_channel_quant_gemm(
        X_q.cpu(),
        W_q.cpu(),
        w_scales.cpu(),
        x_scale.cpu(),
    ...
    mem_access_bytes = (
        ...
        + w_scales.element_size() * w_scales.nelement()
    ...
    for name, func in funcs_to_bench.items():
        ...
        bench_by_secs(
            10,
            lambda: func(X_q, W_q, out, w_scales, x_scale),
