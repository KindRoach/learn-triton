...
def ref_moe_gemm_explicit_group(
    ...
    for e in range(E):
        ...
        gate_up = x_e @ w_gate_up[e]
        act = F.silu(gate_up[:, :I]) * gate_up[:, I:]
        y_e = act @ w_down[e]

        out[start:end] = y_e  # [N_e, H]
...
def main():
    ...
    # accuracy check
    out_implicit = ref_moe_gemm_implicit_group(hiddens, topk_expert_ids.squeeze(1), w_gate_up, w_down)
    out_explicit = ref_moe_gemm_explicit_group(reordered_hiddens, expert_offsets, w_gate_up, w_down)
    ...
    funcs_to_bench = {
        ref_moe_gemm_implicit_group.__name__: lambda: ref_moe_gemm_implicit_group(
            hiddens, topk_expert_ids.squeeze(1), w_gate_up, w_down
        ),
